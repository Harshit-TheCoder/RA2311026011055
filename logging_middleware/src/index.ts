...
export async function Log(
  ...
  if (
    ...
    console.warn(
      "Log Warning: stack, level, and package must be in lower case.",
    );
  }
  ...
  if (!token) {
    console.error(
      "Log Error: No access_token found. Please run the setup_auth script first.",
    );
  ...
  try {
    ...
    if (response.status === 200 || response.status === 201) {
      console.log(
        `[${level.toUpperCase()}] [${stack}] [${pkg}]: ${message} (LogID: ${response.data.logID})`,
      );
    } else {
      console.error(
        `Log API Error: Unexpected status code ${response.status}`,
        response.data,
      );
    }
  } catch (error: any) {
    console.error(
      `Log API Error: Failed to send log to Test Server`,
