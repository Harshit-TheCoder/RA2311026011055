...
async function setup() {
  ...
  try {
    ...
    console.log(
      "Registration failed or already registered. Attempting to use existing credentials...",
    );
    if (fs.existsSync(AUTH_FILE_PATH)) {
      try {
        const existingAuth = JSON.parse(
          fs.readFileSync(AUTH_FILE_PATH, "utf8"),
        );
    ...
    if (!clientID || !clientSecret) {
      console.error(
        "Could not find clientID/clientSecret. Please ensure they are in auth.json.",
      );
      return;
    }
  }
